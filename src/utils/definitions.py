...
def initialize_data(config):
    ...
    if config["train_test"]["train"] or config["train_test"]["test"] or config["train_test"]["evaluate"] or config["train_test"]["evaluate_new_angles"] or config["train_test"]["boundary_test"] or config["plotting"]["make_div_plots"] or config["plotting"]["make_RANS_plots"]:
        if config["training"]["use_PCA"]:
            pca_transformer = get_pca_transformer(config, device)
            data_dict, input_params, output_params = pca_transformer.load_data_PCA()
        elif config['training']['use_fft']:
            import mor
            data_dict = load_data_FFT_full(config, device)
...
def print_statement(epoch, epochs, use_epoch, current_loss, loss_dict, free_memory, current_elapsed_time_hours):
    print(f'Epoch [{epoch}/{epochs if use_epoch else "infinity"}], '
          ...
          f'grad_norm = {loss_dict["grad_norm"]}, '
...
class BasePCATransformer:
    def __init__(self, config, device):
        self.config = config
        self.device = device

    def load_data(self):
        # Load data common to all PCA styles
        pass

    def perform_pca(self):
        raise NotImplementedError("This method should be overridden by subclasses")

class PCAStyle1(BasePCATransformer):
    def __init__(self, config, device):
        super().__init__(config, device)
        print("PCA Style 1 initialized")
        print ("pca with style 1 - features and targets separation - target total size (60 by 60)")
    ...
    def load_data_PCA(config, device):
        ...
        new_input_params = [f"{value}_{angle}" for angle in config["training"]["angles_to_train"] for value in config["training"]["input_params"]]*config["training"]["features_factor"]
        new_output_params = [f"{value}_{angle}" for angle in config["training"]["angles_to_train"] for value in config["training"]["output_params"]]*config["training"]["targets_factor"]
        
        return data_dict, new_input_params, new_output_params
...
class PCAStyle2(BasePCATransformer):
    def __init__(self, config, device):
        super().__init__(config, device)
        print("PCA Style 2 initialized")
        print ("pca with style 2 - features and targets separation - target total size (120 by 60)")
    ...
    def load_data_PCA(config, device):
        ...
        def get_tensor(config, filenames, datafolder_path, wind_angles, angle_to_label, device, pca_reduce, scaler=None, pca=None):
            ...
            pd.DataFrame(reduced_features).to_csv('reduced_features.csv')
            pd.DataFrame(reduced_targets).to_csv('reduced_targets.csv')
    ...
    def invert_data_PCA(config, pred, x):
        ...
        compute_mse(targets, preds_reconstructed, description='TARGETS vs PRED MSE inv PCA')
        return x_reconstructed
...
class PCAStyle3(BasePCATransformer):
    def __init__(self, config, device):
        super().__init__(config, device)

    ###PCA-STYLE-3###
    def get_filenames_from_folder(self, path, extension, startname):
        return [f for f in os.listdir(path) if os.path.isfile(os.path.join(path, f)) and f.endswith(extension) and f.startswith(startname)]
    ...
    def get_features(self, config, training_wind_angles, output_params):
        features_temp = []
        for angle in training_wind_angles:
            column_vector = np.full((len(config["training"]["angles_to_train"])*len(output_params), 1), fill_value=(np.cos(np.deg2rad(angle))))
            features_temp.append(column_vector)
        features = np.vstack(features_temp)
        return features

    def reshaped_array(self, alpha_matrix,training_wind_angles):
        blocks = [alpha_matrix[:, 3*i:3*(i+1)] for i in range(len(training_wind_angles))]  # Slicing out each 36x3 block
        vertically_concatenated = np.vstack(blocks)  # Stacking them vertically
        return vertically_concatenated

    def inverse_reshaped_array(self, vertically_concatenated, training_wind_angles):
        num_blocks = len(training_wind_angles)
        total_rows = vertically_concatenated.shape[0]
        block_height = total_rows // num_blocks
        blocks = [vertically_concatenated[block_height*i:block_height*(i+1), :] for i in range(num_blocks)]
        alpha_matrix_recreated = np.hstack(blocks)
        return alpha_matrix_recreated

    def compute_mse(self, Z, Z_r, description=None):
        mse = mean_squared_error(np.abs(Z), np.abs(Z_r))
        print (f'mse: {mse} - {description}')
        return mse
    ...
    def compute_SVD(self, A):
        ...
        # np.savetxt("sigma_matrix.csv", S_diagonal, delimiter=",")
        # np.savetxt("Vh_matrix.csv", Vh, delimiter=",")
        # np.savetxt("projected_data.csv", projected_data, delimiter=",")

        return projected_data, U, s

    def inv_compute_SVD(self, projected_data, U):
        return np.dot(U, projected_data)

    def concatenate_data_files(self, training_wind_angles, filenames, datafolder_path, output_params):
        dfs = []
        for wind_angle in training_wind_angles:
            for filename in sorted(filenames):
                if int(filename.split('_')[-1].split('.')[0]) == wind_angle:
                    df = pd.read_csv(os.path.join(datafolder_path, filename))
                    print ('Each Dataset has shape - ', df.shape, " - The wind angle - ", wind_angle)
                    df = df[output_params]
                    print ('Each Dataset has shape - ', df.shape, " - The wind angle - ", wind_angle)
                    dfs.append(df)
    ...
    def get_tensor(self, config, training_wind_angles, filenames, datafolder_path, output_params, U):
        targets = self.concatenate_data_files(training_wind_angles, filenames, datafolder_path, output_params)
        targets, target_scaler = self.get_targets(targets)
        
        if U is None:
            alpha_matrix, U, sigma = self.compute_SVD(targets)
        else:
            alpha_matrix = np.dot(U.T, targets)

        self.compute_mse(self.inv_compute_SVD(alpha_matrix, U), targets, 'targets vs targets reconstructed')

        alpha_matrix_reshaped = self.reshaped_array(alpha_matrix, training_wind_angles)

        alpha_matrix_recreated = self.inverse_reshaped_array(alpha_matrix_reshaped, training_wind_angles)

        self.compute_mse(alpha_matrix_recreated, alpha_matrix, 'alpha_matrix - before and after reshape')

        features = self.get_features(config, training_wind_angles, output_params)

        x = [features, alpha_matrix_reshaped, U, targets, target_scaler, training_wind_angles]

        return x

    def load_data_PCA(self):
        print("PCA Style 3 initialized")
        print ("pca with style 3 - features and targets separation - target total size (60 by 60), claude style")
        config = self.config
        device = self.device
        chosen_machine_key = config["chosen_machine"]
        datafolder_path = os.path.join(config["machine"][chosen_machine_key], config["data"]["data_folder_name"])
        ...
        x_training = self.get_tensor(config, training_wind_angles, filenames, datafolder_path, output_params, None)
        ...
        x_skipped = self.get_tensor(config, skipped_wind_angles, filenames, datafolder_path, output_params, x_training[2])
        ...
        new_output_params = output_params
        
        return data_dict, new_input_params, new_output_params

    def inv_reshape_compute_SVD(self, pred, wind_angles, U):
        return self.inv_compute_SVD(self.inverse_reshaped_array(pred, wind_angles), U)

    def inverse_reshaped_array_torch(self, vertically_concatenated, training_wind_angles):
        num_blocks = len(training_wind_angles)
        total_rows = vertically_concatenated.shape[0]
        block_height = total_rows // num_blocks
        blocks = [vertically_concatenated[block_height * i:block_height * (i + 1), :] for i in range(num_blocks)]
        alpha_matrix_recreated = torch.cat(blocks, dim=1)  # Use torch.cat and specify the dimension
        return alpha_matrix_recreated

    def inv_compute_SVD_torch(self, projected_data, U):
        return torch.matmul(U, projected_data)

    def inv_reshape_compute_SVD_torch(self, pred, wind_angles, U):
        return self.inv_compute_SVD_torch(self.inverse_reshaped_array_torch(pred, wind_angles), U)

    def invert_data_PCA(self, config, pred, x):

        [features, alpha_matrix_reshaped, U, targets, target_scaler, wind_angles] = x

        pred_reconstructed = self.inv_compute_SVD(self.inverse_reshaped_array(pred, wind_angles), U)
        pred_reconstructed = inverse_transform_targets(pred_reconstructed, target_scaler)
        targets = inverse_transform_targets(targets, target_scaler)
        assert targets.shape == pred_reconstructed.shape
        x_reconstructed = [targets, pred_reconstructed]
        self.compute_mse(targets, pred_reconstructed, description='TARGETS vs PRED MSE inv PCA')
        return x_reconstructed

    def evaluate_model_training_PCA(self, config, device, model, model_file_path, X_test_tensor, y_test_tensor, x):
        checkpoint = torch.load(model_file_path, map_location=device)
        model.load_state_dict(checkpoint['model_state_dict'])
        model.eval()
        with torch.no_grad():
            ...
            y_test_tensor_cpu = y_test_tensor.cpu()
            X_test_tensor_cpu = X_test_tensor.cpu()
            x_reconstructed = self.invert_data_PCA(config, predictions_tensor_cpu, x)
            [targets_final, targets_pred_final] = x_reconstructed
            mses = []
            r2s = []
            ...
            def reshape_list(config, arr):
                if len(arr) == len(config["training"]["output_params"]):
                    return arr
                else:
                    blocks = [arr[len(config["training"]["output_params"])*i:len(config["training"]["output_params"])*(i+1)] for i in range(len(config["training"]["angles_to_train"]))]
                    arr_ = np.vstack(blocks)
                    averages = np.mean(arr_, axis=0)
                    return list(averages)

            mses = reshape_list(config, mses)
            r2s = reshape_list(config, r2s)

        model.train()
        return mses, r2s

class PCAStyle4(BasePCATransformer):
    def __init__(self, config, device):
        super().__init__(config, device)
        print("PCA Style 4 initialized")
        print ("pca with style 4 - features and targets separation - target total size (60 by 60), claude style modf skipped wind angles")
    ...
    def inverse_reshaped_array(self, vertically_concatenated, training_wind_angles):
        ...
        blocks = [vertically_concatenated[block_height*i:block_height*(i+1), :] for i in range(num_blocks)]
        alpha_matrix_recreated = np.hstack(blocks)
        print (vertically_concatenated.shape, alpha_matrix_recreated.shape, training_wind_angles)
        return alpha_matrix_recreated
    ...
    def get_tensor(self, config, training_wind_angles, filenames, datafolder_path, output_params, U):
        ...
        print (alpha_matrix.shape, alpha_matrix_reshaped.shape, alpha_matrix_recreated.shape)

        self.compute_mse(alpha_matrix_recreated, alpha_matrix, 'alpha_matrix - before and after reshape')

        features = self.get_features(config, training_wind_angles, output_params)

        print (features.shape, features, training_wind_angles)
    ...
    def evaluate_model_training_PCA(self, config, device, model, model_file_path, X_test_tensor, y_test_tensor, x):
        ...
        with torch.no_grad():
            ...
            y_test_tensor_cpu = y_test_tensor.cpu()
            X_test_tensor_cpu = X_test_tensor.cpu()
            print (len(X_test_tensor_cpu),len(y_test_tensor_cpu),len(predictions_tensor_cpu))
            x_reconstructed = self.invert_data_PCA(config, predictions_tensor_cpu, x)
            [targets_final, targets_pred_final] = x_reconstructed
            mses = []
            r2s = []
...
def get_pca_transformer(config, device):
    pca_style = config["training"]["pca_style"]
    if pca_style == "style-1":
        return PCAStyle1(config, device)
    elif pca_style == "style-2":
        return PCAStyle2(config, device)
    elif pca_style == "style-3":
        return PCAStyle3(config, device)
    elif pca_style == "style-4":
        return PCAStyle4(config, device)
    else:
        raise ValueError(f"Unsupported PCA style: {pca_style}")
...
def old_definitions():
    ...
    #     all_params = input_params + output_params
    #     df = df[all_params]
    #     features_targets_orig = np.array(df)
    #     if scaler is None:
    #         scaler = config["training"]["feature_scaler"]
    #         scaler.fit(features_targets_orig)
    #     features_targets = scaler.transform(features_targets_orig)
    #     return features_targets_orig, features_targets, scaler
    ...
    #     print(f"SVD: U - {U.shape}, s - {s.shape}, V_dagger - {Vh.shape}")
    #     # print("Variance explained by each principal component:", variance_explained)
    #     print("Variance explained sum:", np.sum(variance_explained))
    ...
    #     data, labels = concatenate_data_files(filenames, datafolder_path, wind_angles, angle_to_label)
    #     features_targets_orig, features_targets, scaler = get_scaled_features_targets(config, data, config["training"]["input_params"], config["training"]["output_params"], scaler)

    #     features_targets_fft = np.fft.rfftn((reshaped_array(features_targets, len(wind_angles))).T)

    #     if features_targets_fft_eigenvectors is None:
    #         features_targets_fft_eigenvectors = compute_PCA(features_targets_fft)
    ...
    #     reduced_features, reduced_targets = split_features_targets(features_targets_fft_pca.T, len(config["training"]["input_params"]), len(config["training"]["output_params"]))
    #     features, targets = split_features_targets(reshaped_array(features_targets_orig, len(wind_angles)), len(config["training"]["input_params"]), len(config["training"]["output_params"]))
    ...
    #     if pred is not None:
    #         [reduced_features, reduced_targets_, features, targets, features_targets_fft_eigenvectors, scaler, original_shape] = x
    #         pred = pred.reshape(original_shape)
    #         pred = from_real_imag(pred, original_shape)
    #         reduced_targets = pred
    #     else:
    #         [reduced_features, reduced_targets_, features, targets, features_targets_fft_eigenvectors, scaler] = x
    #         reduced_targets = reduced_targets_
    ...
    #     features_targets_reconstructed = np.fft.irfftn(features_targets_fft_reconstructed, s=(features_targets.T).shape)
    ...
    #     features, targets = split_features_targets(features_targets, len(config["training"]["input_params"]), len(config["training"]["output_params"]))
    #     features_reconstructed, targets_reconstructed = split_features_targets(features_targets_reconstructed, len(config["training"]["input_params"]), len(config["training"]["output_params"]))
    ...
    #     X_train_tensor, y_train_tensor, original_shape_train = get_real_tensor(x_training[0], x_training[1], device)

    #     X_test_tensor_skipped, y_test_tensor_skipped, original_shape_skipped = get_real_tensor(x_skipped[0], x_skipped[1], device)
