...
from src.visualization.plotting_definitions import *
...
def make_singular_plot(figsize, z, grid1, grid2, grid3, geometry, quiver_points1, quiver_points2, quiver_v1, quiver_v2, plane, wind_angle, cut, tolerance, cbar_label, savename, arrow1= None, arrow2 = None, forced_z=None):
    coordinate1, coordinate2, coordinate3, lim_min1, lim_max1, lim_min2, lim_max2 = get_plane_config(plane)
    fig, ax = plt.subplots(figsize=figsize, sharey=True)
    vmin, vmax, levels, cmap, scatter_size, ticks = plotting_details(z)
    if forced_z is None:
        contour = ax.contourf(grid1, grid2, grid3, levels=levels, vmin=np.min(z), vmax=np.max(z), cmap=cmap)
    else:
        contour = ax.contourf(grid1, grid2, grid3, levels=levels, vmin=np.min(forced_z), vmax=np.max(forced_z), cmap=cmap)
...
def plot_data(config, df, plane, wind_angle, cut, tolerance, geometry_filename, savenames, df_forced=None):
    coordinate1, coordinate2, coordinate3, lim_min1, lim_max1, lim_min2, lim_max2 = get_plane_config(plane)
    filtered_df = filter_dataframe(config, df, wind_angle, coordinate3, cut, tolerance)
    ...
    for grid3 in all_grids:
        if df_forced is not None:
            make_singular_plot((32, 16), filtered_df[savenames[counter][0]].values, grid1, grid2, grid3, geometry, quiver_points1, quiver_points2, quiver_v1, quiver_v2, plane, wind_angle, cut, tolerance, savenames[counter][1], savenames[counter][2], forced_z=df_forced[savenames[counter][0]].values)
        else:
        ...
        if counter == 0 and plane == 'X-Y' and config["plotting"]["arrow"][0]:
            arrow1 = make_arrow(plane,config["plotting"]["arrow"][1][0][0],config["plotting"]["arrow"][1][0][1],wind_angle)
            arrow2 = make_arrow(plane,config["plotting"]["arrow"][1][1][0],config["plotting"]["arrow"][1][1][1],wind_angle)
            if df_forced is not None:
                make_singular_plot((32, 16), filtered_df[savenames[counter][0]].values, grid1, grid2, grid3, geometry, quiver_points1, quiver_points2, quiver_v1, quiver_v2, plane, wind_angle, cut, tolerance, savenames[counter][1], f'{savenames[counter][2]}_arrow.png', arrow1, arrow2, forced_z=df_forced[savenames[counter][0]].values)
            else:
...
def plot_data_predictions(config, df, plane, wind_angle, cut, tolerance, geometry_filename, savenames):
    ...
    for grid3_actual, grid3_pred in all_grids:
        ...
        if counter == 0 and plane == 'X-Y' and config["plotting"]["arrow"][0]:
            arrow10 = make_arrow(plane,config["plotting"]["arrow"][1][0][0],config["plotting"]["arrow"][1][0][1],wind_angle)
            arrow20 = make_arrow(plane,config["plotting"]["arrow"][1][1][0],config["plotting"]["arrow"][1][1][1],wind_angle)
            arrow11 = make_arrow(plane,config["plotting"]["arrow"][1][0][0],config["plotting"]["arrow"][1][0][1],wind_angle)
            arrow21 = make_arrow(plane,config["plotting"]["arrow"][1][1][0],config["plotting"]["arrow"][1][1][1],wind_angle)
...
def plot_data_2d(config,df,wind_angles,geometry_filename,plot_folder,single, df_forced=None):
    ...
    for wind_angle in wind_angles:
        for j in params:
            ...
            if single:
                plot_data(config, df, plane, wind_angle, cut, tolerance, geometry_filename, savenames_single, df_forced)
